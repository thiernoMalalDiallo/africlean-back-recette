...
class AuthenticationPage {
     ...
     */
    constructor(navCtrl, controlField, translateService, httpUser, route, session, observableProvider, websocket) {
    ...
    setSessionVariables(user, role) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            yield this.session.setItem('userId', user.objectId);
    ...
    goToHome(userLog) {
        ...
        if (userLog.length > 0) {
            ...
            if (userLog.length === 1) {
                this.httpUser.loged('login/user-loged/', userLog[0].objectId).subscribe((user) => {
                    this.setSessionVariables(user, role).then((_) => {
                        // Signale une nouvelle session
                        this.observableProvider.isAuth.emit(true);
                        this.setPseudoToServer();
